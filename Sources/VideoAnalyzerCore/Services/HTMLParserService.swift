...
// Conditional import for SwiftSoup with fallback implementation
#if canImport(SwiftSoup)
    import SwiftSoup
#else
    // Fallback implementation for when SwiftSoup module is not available
    // This allows the code to compile even if SwiftSoup isn't properly resolved
    public struct Document {
        private let html: String
        public init(_ html: String) throws { self.html = html }
        public func select(_ selector: String) throws -> Elements { return Elements() }
        public func outerHtml() throws -> String { return html }
    }
    ...
    public class Element {
        public func attr(_ attribute: String) throws -> String { return "" }
        public func text() throws -> String { return "" }
        public func outerHtml() throws -> String { return "" }
    }
    ...
    // Note: This is a minimal stub implementation for compilation purposes only
    // In practice, the full SwiftSoup dependency should be properly resolved
#endif
...
public class HTMLParserService: HTMLParsing, VideoDetecting, ContentExtracting {
    ...
    public func extractPublicationDate(from html: String) -> Date? {
        ...
        guard !html.isEmpty else {
            logger.warning("Empty HTML string provided for date extraction")
            return nil
        }
        ...
        do {
            ...
            for metaTag in dateMetaTags {
                do {
                    ...
                    if !content.isEmpty, let date = parseDate(content) {
                        return date
                    }
                } catch {
                    logger.debug("Error accessing meta tag content: \(error)")
            ...
            for timeTag in timeTags {
                do {
                    let datetime = try timeTag.attr("datetime")
                    if !datetime.isEmpty, let date = parseDate(datetime) {
                        return date
                    }
                } catch {
                    logger.debug("Error accessing time tag datetime: \(error)")
                    // Continue to next tag
                }
            }
            ...
            for pattern in datePatterns {
                do {
                    // Proper try-catch for regex compilation
                    let regex = try NSRegularExpression(pattern: pattern)

                    // Use utf16.count instead of utf8.count for NSRange calculations
                    // This is crucial because NSRange works with UTF-16 code units, not UTF-8
                    let range = NSRange(location: 0, length: html.utf16.count)
                    let matches = regex.matches(in: html, options: [], range: range)

                    for match in matches {
                        // Safely check range and convert to Swift Range
                        if match.numberOfRanges > 1 {
                            let captureRange = match.range(at: 1)

                            // Only proceed if range is valid
                            if captureRange.location != NSNotFound,
                               let swiftRange = Range(captureRange, in: html) {
                                let dateString = String(html[swiftRange])
                    ...
                    // Log specific regex compilation or matching errors
                    logger.error("Error with regex pattern '\(pattern)': \(error)")
                }
            }
        } catch {
            logger.error("Error parsing HTML for date extraction: \(error)")
        }
