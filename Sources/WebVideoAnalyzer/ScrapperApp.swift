...
struct ScrapperApp: App {
    ...
    // Default initializer
    init() {
        // Basic initialization - no command-line handling here to avoid concurrency issues
    }
    
    // When the app finishes launching, we'll check for command-line arguments
    // This will be handled through standard App lifecycle methods
    
    /// Handles command-line arguments for the analyzer
    private func handleCommandLineArguments(_ arguments: [String]) async {
        ...
        case "batch":
            ...
            await performBatchAnalysis(filePath: filePath)
        ...
        case "validate":
            ...
            let url = arguments[2]
            await performValidation(url: url)
    ...
    private func performBatchAnalysis(filePath: String) async {
    ...
    /// Validates a URL
    private func performValidation(url: String) async {
